...
║               [BOTONES GLASSMORPHISM NEÓN REDISEÑADOS]              ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
def draw_glass_button(frame, x1, y1, x2, y2,
                      label, icon_char,
                      accent_color,       # color neón del botón (BGR)
                      is_active=False,
                      is_hover=False,
                      hover_progress=0.0,
                      t=0.0,
                      icon_symbol=None):  # SVG-like símbolo alternativo
    """
    Botón glassmorphism con:
    - Fondo translúcido oscuro degradado
    ...
    if is_active:
        # Activo: tinte del color de acento más pronunciado
    ...
    # ── 2. Degradado interno sutil (más claro arriba) ───────────────
    ...
    # ── 5. Icono circular neón ─────────────────────────────────────
    icon_cx = x1 + 26
    icon_cy = (y1 + y2) // 2

    # Fondo del icono: círculo sólido con color de acento
    if is_active:
    ...
    # ── 6. Texto del botón — blanco con sombra para legibilidad ────
    txt_x = x1 + 48
    txt_y = (y1 + y2) // 2 + 6
...
def _stroke_rounded(img, x1, y1, x2, y2, r, color, thickness):
    """Borde de rectángulo con esquinas redondeadas."""
    r = max(r, 1)
...
def draw_glass_sidebar(frame, x_start, width, height, accent_top, accent_bot, t):
    """Panel lateral con efecto glass oscuro y borde neón."""
    # Fondo oscuro semi-transparente
    overlay = frame.copy()
    for y in range(height):
        yf = y / height
        # Degradado oscuro: azul-noche arriba → negro mate abajo
        b = int(28*(1-yf) + 15*yf)
        g = int(22*(1-yf) + 12*yf)
        r = int(38*(1-yf) + 18*yf)
        frame[y, x_start:x_start+width] = (b, g, r)

    # Trama de puntos sutil (textura glass)
    dot_spacing = 12
    for dy in range(0, height, dot_spacing):
        for dx in range(0, width, dot_spacing):
            px, py = x_start + dx, dy
            if 0 <= py < height and 0 <= px < frame.shape[1]:
                cv2.circle(frame, (px, py), 1, (45, 38, 58), -1)

    # Línea de brillo superior
    cv2.line(frame, (x_start, 0), (x_start+width, 0), (200, 180, 220), 1)


def draw_glass_border(frame, x, y1, y2, color, side='left'):
    """Línea divisoria neón entre sidebar y canvas."""
    # Glow multicapa
    for gw in [5, 3, 1]:
        alpha = 0.08 * (6 - gw)
        ov = frame.copy()
...
class VirtualPainter:
    ...
    def print_drawing(self,frame_bg=None):
        ...
        for title,y,fs,th,col in [
            ...
            ("Feria Agroalimentaria, Tecnologica y Turistica Sostenible - Pintura con Gestos de Mano",122,0.55,1,(80,80,80))]:
    ...
    def _draw_ui(self,frame,gesture,fps):
        ...
        # ── Panel título lateral izquierdo ────────────────────────────
        # Fondo del panel título (ya cubierto por sidebar)
        # Marco decorativo
        ...
        # ══════════════════════════════════════════════════════════════
        #  BOTONES IZQUIERDOS — glassmorphism neón
        # ══════════════════════════════════════════════════════════════
        LEFT_BTN_INFO={
        ...
        # ══════════════════════════════════════════════════════════════
        #  BOTONES DERECHOS — glassmorphism neón
        # ══════════════════════════════════════════════════════════════
        RIGHT_BTN_INFO={
        ...
        if col_indicator_y + 30 < H - 55:
            ...
            # Borde neón del círculo de color
        ...
        if self._notif_timer>0:
            ...
            ov=frame.copy()
            # Fondo oscuro de la notificación
...
def _print_banner():
    print("="*68)
    print("   M A G I C   P A I N T  v5.1  -  Botones Glassmorphism Neón")
